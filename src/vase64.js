...
export function base64ToVase(base64, options = {}) {
  ...
  const rowsWanted = Math.ceil(values.length / CELL_COLUMNS);
  const bedWidth = Math.max(
    BED.min,
    Math.min(BED.default + Math.max(0, rowsWanted - 1) * BED.perRow, BED.max),
  );
  const columns = Math.max(1, Math.floor(bedWidth / CELL_COLUMNS));
  ...
  // half-column keeps an even-width grid and an odd-width vase on the identical
  // centre column.
  const leftPad = Math.max(
    0,
    Math.floor((width - 1) / 2 - (bedWidth - 1) / 2),
  );
  const place = (content) => ' '.repeat(leftPad) + content;

  const lines = [];
  const plantRows = Math.ceil(values.length / columns);
  for (let rack = 0; rack < plantRows; rack += 1) {
    const slice = values.slice(rack * columns, (rack + 1) * columns);
    for (let cellRow = 0; cellRow < CELL_ROWS; cellRow += 1) {
      const across = slice.map((value) => GLYPHS[value][cellRow]).join('');
      lines.push(place(across.padEnd(bedWidth)).trimEnd());
    }
  }
